...
class TextFeature_LSTM(nn.Module):
    def __init__(self, nHidden, seqLen, guideLen, textEmbeddingPath, numLayers=1, dropout=0, device='cuda'):
        """
        :param nHidden: 隐藏层
        :param seqLen:  步长
        ...
        if device == "cuda":
            self.embeddingArray = self.embeddingArray.to(self.device)
    ...
    def forward(self, X, guideVector):
        ...
        """
        # guideVector=guideVector.cpu()


        guideVector = guideVector.to(self.device)     ###### 有的话可以跑conf_matrix
        X = X.to(self.device)
...
class TextFeature_Bert(nn.Module):
    def __init__(self, nHidden, sqLen, dropout, device="cuda"):
        """
        :param nHidden:
        :param sqLen:
        :param dropout:
